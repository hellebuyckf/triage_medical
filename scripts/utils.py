...
def check_demo_env() -> None:
    ...
    - ``MLFLOW_TRACKING_TOKEN``: must be set and non-empty (GCP OIDC token,
      generated by ``gcloud auth print-identity-token``).
    - ``gcloud`` in PATH: warning only — required for mid-run token refresh
      (``refresh_mlflow_token``). Training exceeds the 1-hour token TTL.
    ...
    if not token:
        errors.append(
            ...
            "    Run: make train-sft ENV=demo  (the Makefile injects the token automatically)\n"
            "    Or manually: export MLFLOW_TRACKING_TOKEN=$(gcloud auth print-identity-token)"
        )
    ...
    if shutil.which("gcloud") is None:
        warnings.append(
            ...
            "    The token cannot be refreshed mid-run (refresh_mlflow_token will be a no-op).\n"
            "    Install the Google Cloud SDK if training runs exceed 1 hour."
        )
